...
def get_top_processes_by_memory(num_processes=10):
    ...
    for proc in psutil.process_iter(
        ...
        try:
            ...
            ):  # Ensure memory_percent is not None
                proc_info["cmdline"] = (
...
def display_system_info(screen, mem, swap, cpu, processes):
    ...
    name_width = max(
        20, (width - 80) // 2
    )  # Minimum 20 characters, or half the available space
    cpu_width = 6
    mem_width = 6
    cmdline_width = max(
        60, width - pid_width - name_width - cpu_width - mem_width - 4
    )  # Minimum 60 characters or remainder space
